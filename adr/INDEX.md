...
| 078 | One Conventions File per Platform, in `config/conventions/` | `config/conventions/<platform>.yaml` composes into one `Conventions`; the filename is the platform id so no merge rule is needed, and it is the only layout. The bridge server consolidates onto `ConfigLoader`, and `specs init` scaffolds `figma.yaml` alone |
