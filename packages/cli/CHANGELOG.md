...
- **Every generated rule block says why it exists.** A variant block is labelled with the configuration it came from (`/* Variant: size=large, appearance=filled */`), and the cursor affordances name the states classification that declared them — so a reader can trace a block in the stylesheet back to the statement in the spec that produced it. Blocks sharing a selector stay separate rather than being merged: each is a distinct statement, and merging them would lose which statement a declaration belongs to.
...
- **An unreachable license server and a failed license check report differently.** The first says the server was never reached and points at the network, any proxy, and TLS inspection; the second says the server answered but the check did not complete and says to retry. Both say the key was not validated and that no licensed output was written. Telling someone offline to retry in a few seconds sends them looking in the wrong place.
- **`analyze props` reports an absent `nullable` as the per-type default ADR-065 documents** — true for a string, number, slot or image prop, false for an enum or boolean. It tested for an explicit `true`, which reported the opposite of the default for essentially every non-enum prop.
