...
function bump(dir: string, override: { class: 'major' | 'minor' | 'patch'; reason: string } | null = null) {
  const ws = resolveWorkspace(dir);
  const plan = planBump(ws, ruleSet, override);
  if (plan.fatal.length === 0 && !(plan.noChanges && !plan.initialized)) {
    commitBump(ws, plan, { reportMd: '# report\n', changelogMd: '# changelog\n' });
  }
  return plan;
}

describe('bump — initialization', () => {
  it('initializes every component at 1.0.0 and the library at 1.0.0', () => {
    ...
    expect(library.versions[0].version).toBe('1.0.0');
    ...
    expect(button.versions[0].version).toBe('1.0.0');
    ...
    expect(fs.existsSync(path.join(dir, 'versions/1.0.0/specs/deButton/api.yaml'))).toBe(true);
    expect(fs.existsSync(path.join(dir, 'versions/1.0.0/report.md'))).toBe(true);
    expect(fs.existsSync(path.join(dir, 'versions/1.0.0/changelog.md'))).toBe(true);
  ...
  it('is a no-op when nothing changed since the last version', () => {
    ...
    bump(dir);
    const second = bump(dir);
    expect(second.noChanges).toBe(true);
    expect(readLibraryLedger(path.join(dir, 'versions'))!.versions).toHaveLength(1);
  });
});

describe('bump — classification drives version movement', () => {
  it('bumps a component MINOR and the library MINOR on an optional prop addition', () => {
    ...
    expect(plan.libraryFrom).toBe('1.0.0');
    expect(plan.libraryVersion).toBe('1.1.0');
    expect(plan.libraryBump).toBe('minor');

    const button = readComponentLedger(path.join(dir, 'versions'), 'deButton')!;
    expect(button.versions.map(v => v.version)).toEqual(['1.0.0', '1.1.0']);
    expect(button.versions[1].changeType).toBe('minor');
    expect(button.versions[1].libraryVersion).toBe('1.1.0');
    ...
    expect(fs.existsSync(path.join(dir, 'versions/1.1.0/specs/deButton/api.yaml'))).toBe(true);
  ...
  it('bumps MAJOR on a prop removal; variants-only changes stay PATCH', () => {
    ...
    expect(button.to).toBe('2.0.0');
    expect(alert.changeType).toBe('patch');
    expect(alert.to).toBe('1.0.1');
  });
  ...
  it('tracks removed and added components', () => {
    ...
    expect(plan.libraryVersion).toBe('2.0.0'); // removal is major
    ...
    expect(chip.versions[0].version).toBe('1.0.0');
  });
});

describe('bump — identity', () => {
  it('continues history through a tracked component rename', () => {
    ...
    expect(renamed.from).toBe('1.0.0');
    expect(renamed.to).toBe('2.0.0');
  ...
  it('fails the bump on an untracked title change with matching nodeId (likely rename)', () => {
    ...
    const forced = bump(dir, { class: 'major', reason: 'confirmed: genuinely a new component' });
...
describe('bump — assets', () => {
  it('grades asset added MINOR, unreferenced removal PATCH, and content change PATCH', () => {
    ...
    expect(fs.existsSync(path.join(dir, 'versions/1.1.0/assets'))).toBe(false);
  });
  ...
  it('a --force override records the defect and proceeds', () => {
    ...
    bump(dir);
    fs.removeSync(path.join(dir, 'assets/icons/plus.svg'));
    const plan = bump(dir, { class: 'major', reason: 'icon retired deliberately' });
...
describe('bump — manual overrides', () => {
  it('records --force-minor on a variants-only change and bumps accordingly', () => {
    const dir = workspace('force');
    bump(dir);
    editYaml(dir, 'specs/deButton/variants.yaml', doc => {
      doc.default.elements.root.styles.backgroundColor = { $token: 'DE Color/Action/Loud', $type: 'color' };
    });
    const plan = bump(dir, { class: 'minor', reason: 'visual break' });

    expect(plan.libraryVersion).toBe('1.1.0');
    const button = readComponentLedger(path.join(dir, 'versions'), 'deButton')!;
    expect(button.versions[1].version).toBe('1.1.0');
    expect(button.versions[1].changeType).toBe('minor');
...
describe('restore path — specPathAtVersion', () => {
  it('resolves a component version to the library folder that shipped it', () => {
    ...
    const v1 = specPathAtVersion(versionsDir, ledger, 'deButton', '1.0.0');
    expect(v1.entry.libraryVersion).toBe('1.0.0');
    expect(fs.readFileSync(path.join(v1.dir, 'api.yaml'), 'utf8')).not.toContain('rounded');

    const v11 = specPathAtVersion(versionsDir, ledger, 'deButton', '1.1.0');
    expect(v11.entry.libraryVersion).toBe('1.1.0');
