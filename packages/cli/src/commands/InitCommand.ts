 ...
 * Init Command
 * 
 * Scaffolds a .anova.config.yaml file with production-ready defaults
 * and inline documentation for getting started with Anova CLI.
 */
...
export const Init = new Command('init')
  .description('Initialize a .anova.config.yaml file with production defaults')
  .option('--force', 'Overwrite existing config without prompting')
  .option('-c, --config <path>', 'Custom path for config file (default: .anova.config.yaml)')
...
async function initCommand(options: { force?: boolean; config?: string }): Promise<void> {
  const configPath = options.config || '.anova.config.yaml';
  ...
  try {
    ...
    console.log('   3. Run: specs audit data/library.file.json -o components.md');
    console.log('   4. Run: specs batch components.md -o specs.yaml');
    console.log('');
