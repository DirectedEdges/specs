...
PropBindings can also appear as style values anywhere a [`Style`](styles.md#values) is accepted.

## Further Reading

- [ADR 008 — Introduce PropBinding to Replace ReferenceValue](../../adr/008-prop-bindings.md) — introduces `PropBinding` with `$binding` to avoid JSON Schema `$ref` key collision
