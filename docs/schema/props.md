...
Inferred from Figma variant values when [`inferNumberProps`](config.md#processing) is enabled.
...
Slot constraint properties (`minItems`, `maxItems`, `anyOf`) are emitted when [`slotConstraints`](config.md#processing) is enabled in config.
...
- [ADR 027 — Code-Only Props](../../adr/027-code-only-props.md) — surfaces Figma code-only props with `$extensions` source metadata
- [ADR 028 — Slot Quantity and Content Constraints](../../adr/028-slot-constraints.md) — adds `minItems`, `maxItems`, `anyOf` to SlotProp
- [ADR 029 — NumberProp](../../adr/029-number-prop.md) — adds the `NumberProp` type with opt-in inference
