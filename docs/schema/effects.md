...
An `Effects` object holds shadow and blur definitions. It appears on the `effects` style property as an alternative to a [`TokenReference`](token-reference.md).
...
| `color` | `string \| TokenReference` | Yes | Shadow color (`#RRGGBBAA` or [token](token-reference.md)) |
...
- [ADR 002 — Replace effectStyleId with effects](../../adr/002-effects-shadows-blurs.md) — introduces the grouped `Effects`, `Shadow`, and `Blur` types
