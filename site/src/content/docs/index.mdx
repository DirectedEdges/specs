...
<Image src={specsWorkflow} alt="Diagram titled 'Specs workflow'. A legend distinguishes scripted steps, shown in orange, from agentic steps, shown in blue. On the left, a Figma icon feeds the Specs 2 Plugin and the Specs CLI's fetch-scan-generate commands, both scripted; both produce Specs data, shown as a central database icon. From Specs data, a scripted arrow leads to Specs CLI transform, which produces starter code (CSS, TSX, contract, Stories, and more); an agentic arrow carries that starter code onward to a Prototype Library as well as React, Android, and iOS libraries. Specs data also connects bidirectionally, in agentic blue, to a node labeled Agents, who likewise produce the React, Android, and iOS libraries. Below, a scripted arrow leads from Specs data to Specs CLI analyze, which produces library data (tokens, props, and more), which in turn produces reports on drift and quality, shown via an agentic arrow. Completing the loop, Specs CLI write, via the write bridge, takes Specs data and writes components back into the original Figma file, shown by a scripted arrow returning to the Figma icon." />
