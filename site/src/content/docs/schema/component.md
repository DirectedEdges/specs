...
| `images` | `Record<string, string>` | No | Registry of image data keyed by id, referenced by `Styles.backgroundImage`, `ImageBinding` examples, and `ImageProp` defaults. Each value is a `data:` URI, external URL, asset path, or `figma:<imageRef>` placeholder. Emitted when [`processing.images`](/specs/schema/config/#processingimages) is configured |
