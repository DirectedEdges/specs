...
The same component, in the two surfaces it passes through before a scaffold exists.
...
Two annotations, doing two jobs. The badges name what each layer *is* — a layer called `Container` is a textbox, one called `Value` is the value it holds — which is what a scaffold cannot infer from a name or a fill. The properties panel is the variant and text properties, which become the props.

### Specs

`specs generate` writes both into `api.yaml`. The `anatomy` block is the layer tree with its roles; `props` (not shown) is the panel.
